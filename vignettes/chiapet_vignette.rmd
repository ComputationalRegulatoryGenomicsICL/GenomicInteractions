...
First we need to load the GenomicInteractions package:
...
<!--
The data is subsetted from the original to only include anchors on chromosomes 
17 and 18. -->
...
The two linked `GRanges` object can be returned, but not altered:
...
`GenomicInteractions` objects can easily handle *trans*-chromosomal interactions, 
since the anchors can be from any point along the chromosome. `is.trans` returns
a logical vector, likewise `is.cis` is the opposite of this function.
...
Since genes can have multiple transcripts, they can also have multiple
promoters/terminators, so these are `GRangesList` objects.
...
We can use these to subset our `GenomicInteractions` object directly from
...
summarise interactions according to these features. 
...
overlapping ranges in `annotation.features` will be labelled with `distal`.
...
We can now find promoters using the annotated `node.class` for each anchor:
...
Common `node.class` values have shorter functions:

```{r short_types, eval=F}
k562.rep1[is.pp(k562.rep1)]
k562.rep1[is.dd(k562.rep1)]
k562.rep1[is.tt(k562.rep1)]
```

Summary plots of interactions classes can easily be produced:
...
The summariseByFeatures function summarises all interactions at a defined set
of annotated loci.  In this example we use promoters, however any dataset can
be used, for example transcription-factor ChIP data or predicted cis-regulatory
sites.

The categories are generated from the annotated categories in the object.
...
This allows us to ask questions of the data, and provide a quick method to
isolate genes of interest. In this case we produce lists of RefSeq IDs, which
can easily be converted to EntrezIDs or gene symbols through existing
BioConductor packages.

Which promoters have the strongest Promoter-Promoter interactions?
